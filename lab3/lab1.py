...
def check_user(username, password):
    global misses
    user_data = read_user_data(read_session_key())
    ...
    if user_data[login]["password"] == old_pass:
        if pass1 == pass2:
            ...
            if result[0] and result[1] and result[2]:
                ...
                write_user_data(read_session_key(), user_data)
...
def keychecker():
    ...
    for disk_drive in psutil.disk_partitions(all=False):
        if os.getcwd()[:3] in disk_drive:
            filesystem_type = disk_drive[2]

    data = [username, sysname, path_to_windows,
            path_to_windows_system_fiels,
            screen_size, mouse, filesystem_type,
            size_of_disk]
    print(data)
...
# Open the temporary file and decrypt the data
def decrypt_file(key):
    with open(db_file, "rb") as f_temp, open(temp_file, "wb") as f_out:
        iv = f_temp.read(16)
...
def encrypt_file(key):
    ...
    cipher = AES.new(key, AES.MODE_CBC, iv)
    with open(temp_file, "rb") as f_in, open(db_file, "wb") as f_temp:
        f_temp.write(iv)
...
def func_check_session_key(key, is_new):
    if key == "":
        messagebox.showinfo("Info", "You need to write passphrase!!!")
        first_initializing()
    else:
        if is_new:
            messagebox.showinfo("Info", "New passphrase set up!")
            session_key = generate_session_key(key)
            write_session_key(session_key)
            print(read_user_data(read_session_key()))
            leave()
        else:
            if check_session_key(generate_session_key(key)):
                messagebox.showinfo("Info", "Passphrase is correct!")
                print(read_user_data(read_session_key()))
                leave()
            else:
                messagebox.showerror("error", "Passphrase is wrong!")
                first_initializing()
...
def first_initializing():
    if read_session_key():
        ...
        login_button = tk.Button(root, text="Check passphrase",
        ...
        secret_phrase_entry = tk.Entry(root, show="*")
        login_button = tk.Button(root, text="Set up a new passphrase",
